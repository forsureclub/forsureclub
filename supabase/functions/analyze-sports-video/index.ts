...
serve(async (req) => {
  ...
  try {
    const { videoUrl, playerLevel = "intermediate", focusArea = "" } = await req.json();

    if (!videoUrl) {
      return new Response(
        JSON.stringify({ 
          error: "Missing video URL",
          analysis: generateFallbackAnalysis(playerLevel, focusArea) 
        }),
    ...
    console.log(`Analyzing padel video: ${videoUrl} for player level: ${playerLevel}, focus area: ${focusArea}`);

    // Creating a more specific and structured prompt for padel coaching feedback
    const systemPrompt = `You are an expert padel coach with years of experience training professionals. 
    ...
    const userPrompt = `Analyze this padel video from a ${playerLevel} level player and provide coaching feedback on technique, form, and areas for improvement. 
    ${focusArea ? `The player specifically wants feedback on their ${focusArea}.` : ''}
    The video URL is: ${videoUrl}
    
    Since you can't actually see the video, simulate an analysis for a ${playerLevel} padel player with some common technique issues.
...
function generateFallbackAnalysis(playerLevel = "intermediate", focusArea = "") {
  const padelSpecific = {
    strengths: [
...
As your virtual padel coach analyzing a ${playerLevel} level player${focusArea ? ' focusing on ' + focusArea : ''}, I can see you have ${playerLevel === "beginner" ? "enthusiasm and potential" : playerLevel === "intermediate" ? "good fundamentals" : "strong skills"} but there are some key areas we can improve to take your game to the next level.

## Strengths
- ${padelSpecific.strengths[0]}
- ${padelSpecific.strengths[1]}
- ${padelSpecific.strengths[2]}

## Areas for Improvement
- ${padelSpecific.improvements[0]}
- ${padelSpecific.improvements[1]}
- ${padelSpecific.improvements[2]}

## Recommended Drills
- ${padelSpecific.drills[0]}
- ${padelSpecific.drills[1]}`;
}
